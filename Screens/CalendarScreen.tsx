...
function CalendarScreen({navigation}: any) {
  //TODO add typescript to navigation
  const [data, setData] = useState<TBookingUpdated[] | null>(null);
  const [reservationPeriodsStates, setReservationPeriodsStates] = useState<
    {id: string; periods: periods[]}[] | null
  >(null);
  ...
  const templatePeriod = {
    [formattedDate]: {
      periods: [{startingDay: true, endingDay: false, color: 'pink'}],
    },
  }; //{color: 'transparent'},
  ...
  const globalCalendarData = useSelector(
    (state: RootState) => state.calendarData,
  );
  ...
  const checkDayReservation = (
    day: string,
    globalCalendarData?: TCalendarData,
  ): boolean => {
    ...
    if (globalCalendarData?.data && selectedDay in globalCalendarData.data) {
      console.info(
        'Reservation data exists for the selected day:',
        globalCalendarData.data[selectedDay],
      );
      return true;
    } else {
  ...
  const navigateToDayDetails = (day: string) => {
    ...
    if (checkDayReservation(selectedDay, globalCalendarData)) {
      ...
      navigation.navigate('CurrentBookingsScreen', {selectedDay: selectedDay});
    } else {
  ...
  return (
    <View style={styles.container}>
      <View style={styles.topcontainer}>
        ...
        <Button
          title="populateCalendar()"
          onPress={() => {
            dispatch(UPDATE_STATE(shit2)), setMarkedDates(shit2);
          }}
        />
      </View>
      <Calendar
