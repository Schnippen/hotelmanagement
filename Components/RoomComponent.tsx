...
const RoomComponent = ({item, navigation}: {item: TRoom; navigation?: any}) => {
  ...
  const buttonDisabled =
    room_statusLowerCase === 'occupied' ||
    room_statusLowerCase === 'under maintenance';
  const URL =
    'https://awildgeographer.files.wordpress.com/2015/02/john_muir_glacier.jpg';

  //TODO add item type
  const navigateToAddBookingCalendar = (item: any) => {
    return navigation.navigate('RoomDetailsScreen', {roomDetails: item});
  };

  return (
    <Card>
      <Card.Title>
        {floor_number}, Room: {room_number} {roomClassName}
      </Card.Title>
      <Card.Divider />
      <Image
        source={{uri: URL}}
        style={{aspectRatio: 1, width: '100%', height: 150, flex: 1}}
      ...
      <Button
        ...
        type="outline"
        onPress={() => {
          navigateToAddBookingCalendar(item);
        }}
      />
    </Card>
  );
